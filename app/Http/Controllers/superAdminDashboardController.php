...
class superAdminDashboardController extends Controller
{
    ...
    public function index($id = null) {
        if ($id == null) {
          //Get Logged in User Data from Auth
            ...
            // $currentDay = date('d');
            $dashboard['dailyClients'] = DB::table("client")
                                        ...
                                        ->where('client.created_at', '>=', Carbon::today())
                                        ->get();

            $currentMonth = date('m');
            $dashboard['monthlyClients'] = DB::table("client")
                                          ->leftjoin('organizationtypes', 'client.OrgID', '=', 'organizationtypes.id')
                                          ->leftjoin('subscriptionpackages', 'client.PackID', '=', 'subscriptionpackages.id')
                                          ->select('client.*', 'organizationtypes.Name as orgName', 'subscriptionpackages.Name as packName')
                                          ->whereRaw('MONTH(client.created_at) = ?',[$currentMonth])
                                          ->get();

            $dashboard['weeklyClients'] = DB::table('client')
                                          ->leftjoin('organizationtypes', 'client.OrgID', '=', 'organizationtypes.id')
                                          ->leftjoin('subscriptionpackages', 'client.PackID', '=', 'subscriptionpackages.id')
                                          ->select('client.*', 'organizationtypes.Name as orgName', 'subscriptionpackages.Name as packName')
            ...
            $dashboard['CatCount'] =  DB::table('categories')
                                    ->where('CID', $user->CID)
                                    ->count();

            $dashboard['CharCount'] =  DB::table('characteristics')
                                    ->where('CID', $user->CID)
                                    ->count();

            $dashboard['DietCount'] =  DB::table('diettype')
                                    ->where('CID', $user->CID)
                                    ->count();

            $dashboard['RecipeCount'] =  DB::table('recipes')
                                      ->where('CID', $user->CID)
                                      ->count();

            $dashboard['FoodCount'] =  DB::table('fooditems')
                                    ->where('CID', $user->CID)
                                    ->count();

            $dashboard['PanelCount'] =  DB::table('foodpanel')
                                      ->where('CID', $user->CID)
                                      ->count();
